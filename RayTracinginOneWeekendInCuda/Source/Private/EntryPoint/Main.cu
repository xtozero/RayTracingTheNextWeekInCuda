...
int main( )
{
	...
	Point3 lookAt( 0, 0, 0 );
	Camera cam( lookFrom, lookAt, 20, aspectRatio, 0.1, 10 );
	...
	dim3 threads( 8, 8 );
	FillCanvas<<<grids, threads>>>( devPixels, world, canvas.Width(), canvas.Height(), cam, 500, 50 );
	...
	canvas.WriteFile( "./image_final.ppm" );
}
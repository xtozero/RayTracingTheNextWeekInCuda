...
{
public:
	__host__ Camera( Point3 lookFrom, Point3 lookAt,  double fov, double aspectRatio, double aperture, double focusDist, double time0 = 0.0, double time1 = 0.0 )
		: m_origin( lookFrom ), m_lensRadius( aperture * 0.5 ), m_time0( time0 ), m_time1( time1 )
	{
	...
	__device__ Ray GetRay( curandState_t* randState, double u, double v ) const
	{
		...
		return Ray( m_origin + offset, 
					...
					RandomDouble( randState, m_time0, m_time1 ) );
	}
